...
## Reanalysis of individual D7 scRNAseq
...
##### For the individual D7, cells were sorted by CD45 expression and in total, 6549 decidual cells (CD45+ and CD45-) were analyzed via 10x-genomics. *The authors do not state how many of each were sorted.*
...
```{r, eval=FALSE}
sc1 = load10X("/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs")
sc1 = autoEstCont(sc1)
out1 = adjustCounts(sc1)
DropletUtils:::write10xCounts("/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/vt221_soupX_filtered", out1) #we shall use results from this run
```
...
soupX_Filtered <- Read10X(data.dir = "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/vt221_soupX_filtered")
str(soupX_Filtered)

dim(soupX_Filtered)

dataDir <- '/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/filtered_feature_bc_matrix'
...
```{r, collapse=TRUE}
vt221_jan <- CreateSeuratObject(counts = data, min.cells = 3, min.features = 200)
Assays(vt221_jan)

class(vt221_jan)

saveRDS(vt221_jan, "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/vt221_soupX_filtered/vt221_jan_SeuratObj.rds")
```
...
saveRDS(VT, "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/vt221_soupX_filtered/PREscrublet_vt221.rds")
```
...
include_graphics('/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/QC_Workflow/scrublet_imgs/vt221_cell_16_output_1.png', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
...
```{r, echo=FALSE, out.width = '70%'}
include_graphics('/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/QC_Workflow/scrublet_imgs/vt221_cell_14_output_2.png', auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
                 error = getOption("knitr.graphics.error", TRUE), dpi = 300)
```
...
```{r, collapse=TRUE}
dim(scrublet_calls <- read.csv("/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/vt221_soupX_filtered/scrublet_calls.csv")) 
...
saveRDS(VT, "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/vt221qc.rds")
```

```{r}
# Load the seurat object into the environment
#VT <- readRDS("/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/vt221qc.rds")
```
...
write_xlsx(md, path = "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/jan_metadata.xlsx")
```
...
write_xlsx(totalcells, path = "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/jan_metadata_totalcells.xlsx")
```
...
write_xlsx(dcd45.markers, path = "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/jan_rnaassay_25pfeature_marker.xlsx")
```
...
dcd45.50markers %>%
    group_by(cluster) %>%
    slice_max(n = 5, order_by = avg_log2FC)
write_xlsx(dcd45.50markers, path = "/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/jan_rnaassay_50pfeature_marker.xlsx")
```
...
cellsPerClust
write.csv(cellsPerClust, file="/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/cellsPerClust.csv")
```
...
for(i in clusters){
  ...
  write.csv(cluster.markers, file=paste0("/Volumes/Porrett2/Morgan_G/Data_Analysis/scAnalysis/JAN_VTintegration/vt221_outs/finalqc/", i, "_Markers_RNA.csv"))
}
```

## Overview of VT scRNA-seq reanalysis from Participant D7 

**QC summary:** - Counts were processed using the standard Seurat (v4.2.0) workflow

    - Number of cells after filtering: 655
